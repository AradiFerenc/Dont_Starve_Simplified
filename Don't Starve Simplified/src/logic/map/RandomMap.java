...
public class RandomMap extends Map {
    ...
    public void loadTiles() {
        ...
        for (int y = 0; y < HEIGHT; y++)
        {
            ...
            {
                tiles[y*WIDTH+x]= getTileType(random.nextInt(Tile.tiles.length));
                //tiles[y*WIDTH+x].setPosition(new Coordinate(x*tiles[y*WIDTH+x].getWidth(),y*tiles[y*WIDTH+x].getHeight()));
                tiles[y*WIDTH+x].moveTile(new Coordinate(x*tiles[y*WIDTH+x].getWidth(),y*tiles[y*WIDTH+x].getHeight()));
            }
        }
    }

    public Tile getTileType(int typeId)
    {
        switch (typeId)
        {
            case 0:
